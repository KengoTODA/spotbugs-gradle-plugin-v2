...
public class SpotBugsTaskFactory {
    ...
    private void generateForJava(
            Project project, List<Provider<SpotBugsTask>> tasks, Action<? super SpotBugsTask> configurationAction) {
        project.getPlugins().withType(JavaBasePlugin.class).configureEach(javaBasePlugin -> {
            ...
            List<Provider<SpotBugsTask>> spotbugsTasks = sourceSets.stream()
                    .map(
                            sourceSet -> {
    ...
    private void generateForAndroid(
        ...
        project.getPlugins().withId("com.android.application", plugin -> {
            List<Provider<SpotBugsTask>> spotbugsTasks = project.getTasks().withType(AndroidJavaCompile.class).stream()
                    .map(
                            task -> {
